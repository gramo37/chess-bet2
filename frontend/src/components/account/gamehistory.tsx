...
const GameHistory: React.FC = () => {
  ...
  const fetchGameHistory = async (page: number) => {
    ...
    try {
      const response = await axios.get(`${BACKEND_URL}/game/transaction-history/${page}`, {
        headers: {
        ...
        {games&&games.length > 0 && (
          <table className="min-w-full bg-white border whitespace-nowrap text-center border-gray-200 rounded-lg shadow-lg">
            <thead className="bg-yellow-500 text-white">
              <tr>
                <th className="py-3 px-4 text-left text-sm font-medium uppercase">Game ID</th>
                <th className="py-3 px-4 text-left text-sm font-medium uppercase">White Player</th>
                <th className="py-3 px-4 text-left text-sm font-medium uppercase">Black Player</th>
                <th className="py-3 px-4 text-left text-sm font-medium uppercase">Status</th>
                <th className="py-3 px-4 text-left text-sm font-medium uppercase">Result</th>
                <th className="py-3 px-4 text-left text-sm font-medium uppercase">Start Time</th>
                <th className="py-3 px-4 text-left text-sm font-medium uppercase">End Time</th>
                <th className="py-3 px-4 text-left text-sm font-medium uppercase">Stake</th>
              </tr>
            </thead>
            <tbody>
              {games.map((game: any) => (
...
const GameComponent = ({game,user}:GameProps)=>{
  ...
  return <tr key={game.id} className="border-t hover:bg-gray-50 transition duration-200 ">
  <td 
